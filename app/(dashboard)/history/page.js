...
import { History as HistoryIcon, CheckCircle2, AlertCircle, Clock, User } from "lucide-react";
    ...
    <PageTransition>
      <div className="mb-6">
        ...
        <p className="text-sm text-[var(--text-muted)] mt-1">
          Chronological log of all AI-powered scan assessments
        </p>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Run your first scan on a patient to see results appear here."
        ...
        <StaggerContainer className="space-y-3">
          {scans.map((scan) => {
            ...
            const scanDate = scan.createdAt?.toDate
              ? format(scan.createdAt.toDate(), "MMM d, yyyy 'at' h:mm a")
              : "Unknown date";

            return (
              <StaggerItem key={scan.id}>
                <Link
                  href={scan.patientId ? `/patients/${scan.patientId}` : "#"}
                ...
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                    isHealthy ? "bg-[var(--color-success-light)]" : "bg-[var(--color-danger-light)]"
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[var(--text-primary)] group-hover:text-[var(--color-primary-700)] transition-colors">
                      {isHealthy ? "Healthy Pattern" : "Parkinson's Indicators"}
                      {patient && <span className="text-[var(--text-muted)] font-normal"> — {patient.name}</span>}
                    </p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <Clock className="w-3 h-3 text-[var(--text-muted)]" />
                      <span className="text-xs text-[var(--text-muted)]">{scanDate}</span>
                    </div>
                  </div>
